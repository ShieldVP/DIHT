...
public:
    ConnectedSuffixArray(const std::string& str1, const std::string& str2) : SuffixArray(str1 + char(1) + str2), len1(str1.size()), len2(str2.size()) {}

    std::string commonSubstringInOrder(size_t number);

private:
    size_t len1, len2;
};

std::string ConnectedSuffixArray::commonSubstringInOrder(size_t number) {
